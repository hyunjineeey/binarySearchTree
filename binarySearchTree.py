...
def insert_floor(floor_tree,floor):
    ...
    floor_tree.root.parent = prev
    if (prev == None):
        floor.floor_num = floor_tree
...
def find_nearest_unhaunted(floor_tree,floor_num):
    ...
    while (cur != None and floor_num != cur.floor_num):
        if (abs(best.floor_num - floor_num) > abs(cur.floor_num - floor_num)):
            best = cur
        if (floor_num < cur.floor_num):
            cur = cur.left
        else :
            cur = cur.right
        print (best)
        print (cur)
    if (cur != None and floor_num == cur.floor_num):
        return cur
    return best
